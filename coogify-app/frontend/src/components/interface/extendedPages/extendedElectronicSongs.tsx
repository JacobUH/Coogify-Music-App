...
export const ExtendedElectronicSongs = ({ title }: Props) => {
  ...
  useEffect(() => {
    ...
    const fetchElectronicSongs = async () => {
      try {
        const response = await axios.get(
          `${backendBaseUrl}/api/home/fetchElectronicSongs`,
          {
            headers: {
  ...
  return (
    <div className="w-full flex flex-col md:gap-4 gap-6 px-2">
      <div className="w-full flex items-center justify-between">
        ...
        <Link to="/ElectronicSongs" className="text-[#9E67E4] text-[15px] font-medium">
          See More
        </Link>
      </div>
      <div className="w-full flex items-center overflow-x-auto overflow-y-auto md:pb-0 pb-5">
        <div className="flex items-center gap-2">
          {ElectronicSongs.map((song: Song) => {
            return (
              <div
                key={song.songName}
