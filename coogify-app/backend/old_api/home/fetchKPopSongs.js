import { selectKPopSongs } from '../../database/queries/dbHomeQueries.js';
...
export default async function handler(req, res) {
  jsonParserMiddleware(req, res, async () => {
    await authenticateMiddleware(req, res, async () => {
      try {
        const songs = await selectKPopSongs();
        if (songs !== false) {
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify(songs));
        } else {
          errorMessage(res, 'Error fetching k-pop songs', 'Error');
        }
      } catch (error) {
        errorMessage(res, error, 'Error fetching k-pop songs');
      }
    });
  });
}