import { updateCardDetails } from '../../database/queries/dbCardQueries.js';
...
export default async function handler(req, res) {
  jsonParserMiddleware(req, res, async () => {
    await authenticateMiddleware(req, res, async () => {
      try {
        const songs = await updateCardDetails();
        if (songs !== false) {
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify(songs));
        } else {
          errorMessage(res, 'Error updating card details', 'Error');
        }
      } catch (error) {
        errorMessage(res, error, 'Error updating card details');
      }
    });
  });
}